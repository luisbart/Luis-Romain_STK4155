...
subplot_info = np.zeros((maxdegree,9))

colors = pl.cm.jet(np.linspace(0,1,maxdegree+1))

fig, ax = plt.subplots()
# fig.suptitle('Ridge regression for different lambda')
...
#####Plots####
    ...
    plt.legend()
    plt.savefig("plots/Ridge_Lasso/Ridge_MSE_lambda=%.0e.png" %lambdas[l],dpi=150)
    plt.show()
...
for l in range(nlambdas):   
    ...
    ax.plot(polydegree, np.log10(estimated_mse_Kfold), label='%.0e' %lambdas[l])
