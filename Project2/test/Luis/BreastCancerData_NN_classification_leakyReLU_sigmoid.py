...
def create_neural_network_keras(n_neurons_layer1, n_categories, eta, lmbd):
    model = Sequential()
    model.add(Dense(n_neurons_layer1, nn.softmax, kernel_regularizer=regularizers.l2(lmbd)))
...
heat5.set(xlabel='learning rate', ylabel ='regularization', title = f"Accuracy score training set (Act:leakyReLU-sigmoid)")
plt.savefig(f"Results/NN/BreastCancer_sigmoid_accuracy_score_train_keras_l2.png", dpi=150)
plt.show()
...
heat5.set(xlabel='learning rate', ylabel ='regularization', title = f"Accuracy score test set (Act:leakyReLU-sigmoid)")
plt.savefig(f"Results/NN/BreastCancer_sigmoid_accuracy_score_test_keras_l2.png", dpi=150)
plt.show()
