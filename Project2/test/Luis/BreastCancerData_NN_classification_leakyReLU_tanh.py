...
def create_neural_network_keras(n_neurons_layer1, n_categories, eta, lmbd):
    model = Sequential()
    model.add(Dense(n_neurons_layer1, LeakyReLU(alpha=0.01), kernel_regularizer=regularizers.l2(lmbd)))
...
plt.savefig(f"Results/NN/BreastCancer_tanh_accuracy_score_train_keras_l2.png", dpi=150)
plt.show()
...
plt.savefig(f"Results/NN/BreastCancer_tanh_accuracy_score_test_keras_l2.png", dpi=150)
plt.show()
